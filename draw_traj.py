...
if __name__ == '__main__':
    ...
    ts = [20, 40, 50, 60, 70, 80, 90]
    for t in ts:


        f_str = str([x for x in list(trace['states'][:,0,t])])
        text_file = open("x{}.txt".format(t), "w")
        text_file.write(f_str)
        text_file.close()
    
        f_str = str([x for x in list(trace['states'][:,1,t])])
        text_file = open("y{}.txt".format(t), "w")
        text_file.write(f_str)
        text_file.close()


        f_str = str([x for x in list(trace['states'][:,0,t] -  trace['states'][:,0,t-1])])
        text_file = open("vx{}.txt".format(t), "w")
        text_file.write(f_str)
        text_file.close()
    
    
        f_str = str([x for x in list(trace['states'][:,1,t] -  trace['states'][:,1,t-1])])
        text_file = open("vy{}.txt".format(t), "w")
        text_file.write(f_str)
        text_file.close()
