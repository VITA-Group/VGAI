...
comm_model='knn'
K_neighbor=10
#python grad_train_data_optimal_location.py --n-times $n_times --n-exp $n_exp --start-idx $start_idx --K $filterLength --exp-name $root_path --mode $mode --radius $radius
...
CUDA_VISIBLE_DEVICES=0 python train_loc.py $arch  --K $filterLength --radius $radius --vinit $vinit  --F $n_vis --comm-model $comm_model --mode $mode  --K-neighbor $K_neighbor
